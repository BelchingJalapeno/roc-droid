...
class SenderReceiverService : Service() {
    ...
    fun startSender(ip: String) {
        ...
        senderThread = Thread(Runnable {
            ...
            org.rocstreaming.roctoolkit.Context().use { context ->
                ...
                updateNotification(false, isReceiverRunning())
            }
        })
        ...
        if (isForegroundRunning) {
            updateNotification(true, isReceiverRunning())
        } else {
            startForegroundService(true, isReceiverRunning())
        }
    }
    ...
    fun startReceiver() {
        ...
        receiverThread = Thread(Runnable {
            ...
            updateNotification(isSenderRunning(), false)
        })

        receiverThread!!.start()

        if (isForegroundRunning) {
            updateNotification(isSenderRunning(), true)
        } else {
            startForegroundService(isSenderRunning(), true)
        }
    }
